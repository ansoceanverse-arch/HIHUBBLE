    ...
    if (typeof mediaUrl === 'string' && mediaUrl.startsWith('data:')) {
      try {
        ...
        if (matches && matches.length === 3) {
          ...
          const isVideo = finalMimeType.startsWith('video/') || mediaType === 'video';
          const isAudio = finalMimeType.startsWith('audio/') || mediaType === 'voice' || mediaType === 'audio';
          const defaultExt = isVideo ? 'mp4' : (isAudio ? 'webm' : 'png');
          const ext = finalMimeType.split('/')[1]?.split('+')[0] || defaultExt;
          const filename = `${targetConvId}/${currentUserId}/${Date.now()}_${Math.random().toString(36).substring(7)}.${ext}`;

          const { error: uploadErr } = await supabase.storage
            .from('chat-media')
            .upload(filename, buffer, { contentType: finalMimeType, upsert: true });

          if (uploadErr) {
            console.error('Supabase Storage chat-media upload error:', uploadErr);
            return res.status(500).json({ error: isAudio ? 'Audio upload failed.' : (isVideo ? 'Video upload failed.' : 'Image upload failed.') });
          }

          const { data: publicUrlData } = supabase.storage.from('chat-media').getPublicUrl(filename);
          if (publicUrlData?.publicUrl) {
            finalMediaUrl = publicUrlData.publicUrl;
            finalMediaType = isAudio ? 'audio' : (isVideo ? 'video' : (finalMimeType.startsWith('image/') ? 'image' : (mediaType || 'file')));
          }
        }
      } catch (err) {
