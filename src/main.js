...
window.saveCurrentDraft = async function (silent = false) {
  ...
  if (window.HubbleEditor && window.chUploads && window.chUploads[window.HubbleEditor.activeMediaIndex]) {
  ...
  const draftTitle = captionVal || (primaryItem ? (primaryItem.name || 'Story HUBB') : 'Untitled HUBB');
  ...
  if (totalCount === 0) {
    ...
    drafts.slice(0, 4).forEach(d => {
      ...
      if (list) {
        ...
        item.style.cssText = 'display:flex; justify-content:space-between; align-items:center; background: rgba(255,255,255,0.03); padding: 10px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.06); cursor: pointer; transition: all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1);';
        ...
        item.innerHTML = `
          <div style="display:flex; gap:10px; align-items:center; min-width:0; flex:1;">
            ...
            <div class="ch-draft-info" style="min-width:0; flex:1;">
              <div class="ch-draft-title" style="font-weight:600; font-size:0.85rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--text-main);">${title}</div>
              <div class="ch-draft-time" data-timestamp="${d.lastModified || d.createdAt}" style="color:var(--text-muted); font-size:0.75rem; margin-top:2px;">Saved ${timeStr}</div>
            </div>
          </div>
      ...
      if (hddList) {
        ...
        hItem.style.cssText = 'display:flex; justify-content:space-between; align-items:center; background: rgba(255,255,255,0.03); padding: 10px; border-radius: 12px; margin-bottom: 8px; cursor: pointer; transition: all 0.2s ease; border: 1px solid transparent;';
        hItem.onclick = (e) => {
          if (e.target.closest('button')) return;
          ...
          <div style="display:flex; gap:12px; align-items:center; min-width:0; flex:1;">
            ...
            <div class="ch-draft-info" style="min-width:0; flex:1;">
              <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px;">
                <div class="ch-draft-title" style="font-weight:600; font-size:0.9rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text-main);">${title}</div>
                <span style="font-size: 0.6rem; background: rgba(255,255,255,0.1); color: var(--text-muted); padding: 2px 6px; border-radius: 4px;">Draft</span>
              </div>
              <div class="ch-draft-time" data-timestamp="${d.lastModified || d.createdAt}" style="color:var(--text-muted); font-size:0.75rem;">Saved ${timeStr}</div>
            </div>
          </div>
...
window.renderSeeAllDrafts = async function (query = '') {
  ...
  if (filtered.length === 0) {
    modalList.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:48px 20px; text-align:center; color:var(--text-muted);">
        ...
        <h4 style="margin:0 0 6px 0; color:var(--text-main); font-size:1.1rem;">${query ? 'No matching drafts found' : 'No Story Drafts Yet'}</h4>
  ...
  filtered.forEach(d => {
    ...
    card.style.cssText = 'display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:16px; padding:14px; gap:16px; transition:all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1);';
    card.onmouseover = () => { card.style.background = 'rgba(255,255,255,0.08)'; card.style.borderColor = 'rgba(168,85,247,0.4)'; card.style.transform = 'translateY(-2px)'; card.style.boxShadow = '0 8px 24px rgba(0,0,0,0.3)'; };
    card.onmouseout = () => { card.style.background = 'rgba(255,255,255,0.04)'; card.style.borderColor = 'rgba(255,255,255,0.08)'; card.style.transform = 'none'; card.style.boxShadow = 'none'; };
    ...
    card.innerHTML = `
      <div style="display:flex; gap:14px; align-items:center; min-width:0; flex:1; cursor:pointer;" onclick="window.loadDraft('${d.id}')">
        ...
        <div style="min-width:0; flex:1;">
          <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
            <h4 style="margin:0; font-size:0.95rem; font-weight:600; color:white; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${title}</h4>
            <span style="font-size:0.65rem; background:rgba(168,85,247,0.15); color:var(--primary); padding:2px 6px; border-radius:6px; font-weight:600; flex-shrink:0;">Story</span>
          </div>
      ...
      <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
        <button onclick="window.loadDraft('${d.id}')" style="display:flex; align-items:center; gap:6px; padding:8px 14px; border-radius:10px; background:linear-gradient(135deg, var(--primary) 0%, #d946ef 100%); border:none; color:white; font-weight:600; font-size:0.8rem; cursor:pointer; transition:all 0.2s; box-shadow:0 4px 12px rgba(168,85,247,0.3);"><i data-lucide="edit-3" style="width:14px; height:14px;"></i> Continue Editing</button>
        <button title="Duplicate Draft" onclick="window.duplicateDraft('${d.id}', event)" style="display:flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:10px; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.1); color:white; cursor:pointer; transition:all 0.2s;"><i data-lucide="copy" style="width:16px; height:16px;"></i></button>
        <button title="Delete Draft" onclick="window.deleteDraft('${d.id}', event)" style="display:flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:10px; background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.25); color:#ef4444; cursor:pointer; transition:all 0.2s;"><i data-lucide="trash-2" style="width:16px; height:16px;"></i></button>
      </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            loadStories().then(() => {
              if (!tryOpenStory()) {
                if (typeof showToast === 'function') {
                  showToast('This story has expired or is no longer available.');
                }
              }
            });
          } else {
      ...
      if (storyData && (storyData._id || storyData.id)) {
        ...
        if (typeof openShare === 'function') {
          ...
          window.openShareModal({
            title: storyData.name || 'Story on Hi-Hubble',
            text: storyData.caption || 'Check out this story on Hi-Hubble!',
  ...
  function renderChatThreadsList() {
    ...
    chatThreads.forEach(thread => {
      ...
      if (lastMsg) {
        const currentUser = getCurrentUser();
        if (currentUser) {
          const effectiveMediaType = (lastMsg.mediaType || lastMsg.media_type || '').toLowerCase();
          if (effectiveMediaType === 'image' || effectiveMediaType.includes('image')) {
            lastTextPreview = '📷 Photo';
          } else if (effectiveMediaType === 'video' || effectiveMediaType.includes('video')) {
            lastTextPreview = '🎥 Video';
          } else if (effectiveMediaType === 'audio' || effectiveMediaType === 'voice' || effectiveMediaType.includes('audio') || effectiveMediaType.includes('voice')) {
            lastTextPreview = '🎙️ Voice Note';
          } else {
            const secretKey = getChatSecretKey(currentUser.id || currentUser._id, uId);
            if (secretKey) {
              let decrypted = decryptMessage(lastMsg.content, secretKey);
              let isStoryShare = false;
              ...
              try {
                ...
                if (parsed && typeof parsed === 'object') {
                  if (parsed.hubType === 'story') {
                    isStoryShare = true;
                    parsedStoryId = parsed.hubId;
                    const isStoryStillActive = isStoryActive(parsed.hubId, parsed.timestamp || lastMsg.createdAt || lastMsg.created_at);
                    isStoryExpired = !isStoryStillActive;
                    decrypted = isStoryStillActive ? (parsed.text || 'Shared a Hub Story') : 'Shared a Story · Expired';
              ...
              if (isStoryShare) {
                console.log('[DM STORY DEBUG] Inbox:', {
                  threadUser: uId,
                  eventId: lastMsg.id || lastMsg._id,
                  type: 'story_share',
                  storyId: parsedStoryId,
                  expired: isStoryExpired,
                  read: (thread.unreadCount || 0) === 0
                });
              }
            }
          }
  ...
  function updateThreadLastMessageInPlace(targetUserId, msg) {
    ...
    const effectiveMediaType = (msg.mediaType || msg.media_type || '').toLowerCase();
    if (effectiveMediaType === 'image' || effectiveMediaType.includes('image')) {
      previewText = '📷 Photo';
    } else if (effectiveMediaType === 'video' || effectiveMediaType.includes('video')) {
      previewText = '🎥 Video';
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          if (parsed.hubType === 'story') {
            const isStoryStillActive = isStoryActive(parsed.hubId, parsed.timestamp || msg.createdAt || msg.created_at);
            previewText = isStoryStillActive ? (parsed.text || 'Shared a Hub Story') : 'Shared a Story · Expired';
          } else if (parsed.text !== undefined) {
  ...
  function createMessageBubbleElement(msg, currentUserId, targetUserId, secretKey) {
    ...
    const rawMediaType = (msg.mediaType || msg.media_type || (msg.attachment && msg.attachment.file_type) || '').toLowerCase();
    const hasMedia = rawMediaType && rawMediaType !== 'text';
    const isVideo = rawMediaType === 'video' || rawMediaType.includes('video');
    const isAudio = rawMediaType === 'audio' || rawMediaType === 'voice' || rawMediaType.includes('audio') || rawMediaType.includes('voice');
    const isImage = (rawMediaType === 'image' || rawMediaType.includes('image') || (!rawMediaType && (msg.mediaUrl || msg.media_url))) && !isVideo && !isAudio;
    ...
    if (isHub) {
            ...
            <div class="bubble-content chat-shared-hub-card chat-expired-story-card">
              ...
              <div class="expired-notice-box">
                ...
                <div class="expired-text-col">
                  <span class="expired-title">${authorName} shared a Story</span>
                  <span class="expired-desc">This story has expired — you missed it.</span>
                </div>
              </div>

              <!-- Expired Status Indicator -->
              <div class="expired-footer-status">
                <span class="expired-status-dot"></span> Story Expired
              </div>
            </div>
          `;
        } else {
            ...
            <div class="bubble-content chat-shared-hub-card" 
              ...
              ${thumbnail ? `
                <div style="position: relative; width: 100%; border-radius: 8px; overflow: hidden; background: #000; display: flex; justify-content: center; align-items: center;">
                  ...
                  ` : `
                    <img src="${thumbnail}" alt="Story Preview" loading="lazy" style="width: 100%; max-height: 180px; object-fit: cover; display: block;" />
                  `}
                </div>
              ` : `
      ...
      if (isImage) {
        const imageUrl = resolveBrowserMediaUrl(msg) || (decryptedText.startsWith('http') || decryptedText.startsWith('data:') ? decryptedText : '');
        ...
        const videoUrl = resolveBrowserMediaUrl(msg) || (decryptedText.startsWith('http') || decryptedText.startsWith('data:') ? decryptedText : '');
        ...
        console.log('[VIDEO_RESOLVE]', {
          messageId: msg._id || msg.id,
          videoUrl,
          rawMediaType,
          storagePath: msg.attachment?.storage_path || msg.media_url
        });
        ...
        displayContent = `
          <div class="bubble-content chat-shared-media-card" style="padding: 4px; background: rgba(255,255,255,0.06); border-radius: var(--radius-md); overflow: hidden; max-width: 280px; position: relative;">
            <video controls preload="metadata" playsinline style="display: block; max-width: 270px; max-height: 260px; width: 100%; height: auto; border-radius: var(--radius-sm); object-fit: cover;" onclick="event.stopPropagation();" onloadedmetadata="console.log('[VIDEO_METADATA_LOADED]', '${msg._id || msg.id}')" oncanplay="console.log('[VIDEO_CAN_PLAY]', '${msg._id || msg.id}')" onerror="console.error('[VIDEO_LOAD_FAILED]', { messageId: '${msg._id || msg.id}', videoUrl: '${videoUrl}', rawMediaType: '${rawMediaType}', storagePath: '${msg.attachment?.storage_path || msg.media_url || ''}', errorCode: this.error ? this.error.code : 'unknown', errorMessage: this.error ? this.error.message : '' });">
        ...
        const audioUrl = resolveBrowserMediaUrl(msg) || (decryptedText.startsWith('http') || decryptedText.startsWith('data:') ? decryptedText : '');
        ...
        displayContent = `
          <div class="bubble-content chat-shared-media-card" style="padding: 8px 12px; background: rgba(255,255,255,0.06); border-radius: var(--radius-md); max-width: 280px; min-width: 220px; display: flex; flex-direction: column; gap: 6px; position: relative;">
            ...
            <audio controls preload="metadata" style="width: 100%; height: 36px; outline: none; border-radius: var(--radius-sm);" onclick="event.stopPropagation();">
  ...
  async function openMediaViewer(messageId) {
    ...
    if (effectiveType === 'image' || effectiveType.includes('image')) {
      ...
      video.addEventListener('loadedmetadata', () => console.log('[VIEWER_VIDEO_LOADED]', messageId));
...
window.publishHubb = async function () {
  ...
  try {
    ...
    if (anySuccess) {
      ...
      if (window.showToast) {
        window.showToast(isScheduled ? 'HUB scheduled successfully! 📅🚀' : 'HUB shared to your story! 🚀');
      }
